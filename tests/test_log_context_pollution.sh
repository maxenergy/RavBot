...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}RavBot 上下文污染自动化检测${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
cd "$BUILD_DIR"
if cmake --build . --target ravbot_tests -j$(nproc) 2>&1 | tee -a "$TEST_LOG"; then
...
if ./ravbot_tests --gtest_filter="ContextPollutionTest.*" --gtest_color=yes 2>&1 | tee -a "$TEST_LOG"; then
...
if ./ravbot_tests --gtest_filter="ComprehensiveContextTest.*" --gtest_color=yes 2>&1 | tee -a "$TEST_LOG"; then
...
if ./ravbot_tests --gtest_filter="AgentLoopTest.AnthropicReplay*" --gtest_color=yes 2>&1 | tee -a "$TEST_LOG"; then
