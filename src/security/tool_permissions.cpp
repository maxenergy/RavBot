...
static const std::unordered_map<std::string, std::vector<std::string>> kGroups =
    {
        ...
        {"mobile_safe",
         {"message", "web_search", "web_fetch", "device_status",
          "runtime_status", "speech_status", "set_capture_enabled", "vibrate",
          "camera_snapshot", "time", "memory_list", "memory_search",
          "memory_get", "memory_write", "memory_delete"}},
        {"all",
         {"read",
          "write",
          "edit",
...
ToolPermissionChecker::ToolPermissionChecker(
  ...
  spdlog::info(
      "ToolPermissionChecker: config.allow.size()={}, config.deny.size()={}",
  ...
  for (const auto& entry : config.allow) {
    ...
    if (entry == "*") {
      ...
      mcp_allow_all_ = true;
      spdlog::info(
          "ToolPermissionChecker: Found wildcard '*', setting allow_all=true");
      continue;
    }
    ...
    if (entry.substr(0, 6) == "group:") {
      ...
      if (it != kGroups.end()) {
        spdlog::info(
            "ToolPermissionChecker: Expanding group '{}' with {} tools",
  ...
  if (config.allow.empty()) {
    ...
    mcp_allow_all_ = true;
    spdlog::info(
        "ToolPermissionChecker: Empty allow list, setting allow_all=true");
  }

  // Log configuration
  spdlog::info(
      "ToolPermissionChecker: allow_all={}, allowed_tools={}, denied_tools={}",
