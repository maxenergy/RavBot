...
- `./build/ravbot_tests --gtest_filter=AgentLoopTest.*` runs one Google Test suite.
...
C++ follows the Google style enforced by [`.clang-format`](/home/rogers/source/develop/RavBot/.clang-format): 2-space indentation, 80-column limit, attached braces, sorted includes, and `snake_case` file names like `session_manager.cpp`. Keep classes in `PascalCase`, functions and variables in `snake_case`, and mirror header/source pairs under `include/ravbot/` and `src/`. TypeScript uses ES modules with explicit `.js` import suffixes where the package already does so; follow existing patterns such as `app-lifecycle.ts`, `config.browser.test.ts`, and `rpc-server.test.ts`.
...
After any code change that affects runtime behavior, complete this verification loop before handoff: rebuild the project, restart `ravbot`, confirm there is only one live `ravbot` process, ask the user to send a Telegram message for validation, then inspect `~/.ravbot/logs/` and the active process logs to confirm behavior and troubleshoot any failure.
